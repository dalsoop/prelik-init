...
pxi은 단순한 CLI가 아니라 시스템 곳곳을 변경합니다. **`pxi uninstall`은 pxi 바이너리만 지웁니다.** 시스템에 남는 변경을 어떻게 정리할지 이 문서에 상세히 적혀 있습니다.
...
| 상황 | 명령 |
|---|---|
| 그냥 pxi만 떼고 시스템은 그대로 | `pxi uninstall` (dry-run) → 확인 후 `--confirm` |
| 설정/스냅샷 까지 모두 삭제 | `pxi uninstall --confirm --purge` |
| LXC/VM/NAS/메일/CF까지 완전 청소 | 이 문서 §2~§9를 순서대로 |

---

## 1. `pxi uninstall`이 실제로 하는 일

- `/usr/local/bin/pxi` + `pxi-*` 도메인 바이너리 19개 + `.pxi.version` 마커 삭제
- `~/.local/bin` 도 같은 패턴으로 처리
- **`--purge`** 추가 시:
  - `~/.config/pxi` (사용자 config)
  - `/etc/pxi` (시스템 config)
  - `~/.local/share/pxi` 또는 `/var/lib/pxi` (도메인 캐시 + recovery snapshots + audit log)
...
pxi으로 만든 LXC/VM은 그대로 살아 있습니다. **디스크 데이터까지 전부 사라집니다.**

```bash
# 1) 살아있는 pxi 출처 LXC 확인
pxi run lxc list                 # 또는: pct list

# 2) 백업 (필수 권장)
pxi run backup now <VMID>        # vzdump 스냅샷
# 또는: vzdump <VMID> --storage local --compress zstd --mode snapshot

# 3) 정지 + 삭제
pxi run lxc stop <VMID>
pxi run lxc delete <VMID> --force   # 디스크까지 삭제

# VM은 pxi run vm delete --force <VMID>
```
...
`pxi run nas mount`는 SMB/CIFS와 NFS 마운트 둘 다 `/etc/fstab`에 영구 등록합니다. 각 경로가 다르니 둘 다 살펴야 합니다.

### 3.1 SMB/CIFS 정리 (credentials 파일 포함)

> ⚠️ **순서가 중요합니다.** fstab 라인을 먼저 지우면 어느 credential 파일이 pxi 것인지 역추적 불가능. 아래 단계는 "캡처 → 편집 → 삭제" 순서로 배치돼 있습니다.
>
> pxi의 CIFS 라인 시그니처: fstab 옵션 필드에 `credentials=/etc/cifs-credentials/…` 경로가 들어갑니다. 이 prefix 경로는 pxi 전용이라 필터로 충분히 정확합니다. 타 관리자도 동일한 디렉토리를 쓰면 아래 단계에서 공유될 수 있으니 `sudo ls /etc/cifs-credentials/`로 사전 확인을 권장합니다.

```bash
# 1) 현재 CIFS 마운트 확인
findmnt --target /mnt | awk '$NF ~ /cifs|smb/'

# 2) pxi SMB fstab 라인 식별 (credentials 경로 단독 필터 — 옵션 순서 무관)
...
# 5) fstab에서 pxi SMB 라인만 제거. sed -i.bak로 백업.
...
NFS는 credentials 파일이 없고, pxi이 등록한 fstab 라인 형식은 다음과 같습니다:

```
<server>:<export>  <mount-point>  nfs  _netdev,nofail  0  0
```

이 필드 조합(type=`nfs` + 옵션=`_netdev,nofail`)이 pxi 시그니처입니다. 타 관리자가 NFS를 수동 등록했고 같은 옵션을 썼다면 구분되지 않으니, **삭제 전 반드시 라인을 사람이 확인**하세요.

```bash
# 1) pxi NFS 라인 식별 (type 필드가 3번째 컬럼, 옵션이 4번째)
...
# 3) fstab에서 pxi NFS 라인만 제거 (awk 필터링 + root 소유 임시파일로 atomic replace)
...
`pxi run mail postfix-relay`가 `main.cf`를 수정하고 `pxi-backup-<ns-ts>/` 백업 디렉토리를 만들었습니다.

```bash
# 1) 백업 디렉토리 확인
sudo ls /etc/postfix/pxi-backup-*

# 2) 가장 최근(또는 pxi 이전) 백업으로 복원
...
**경고:** 백업 디렉토리가 여러 개면 가장 첫 백업을 써야 pxi 도입 이전 상태가 됩니다. timestamp 가장 작은 것.

---

## 5. Traefik (리버스 프록시)

`pxi run traefik recreate`로 띄운 LXC + 라우트가 남아 있습니다.

```bash
# Traefik LXC가 별도면 §2 절차로 LXC 삭제
pxi run lxc list | grep traefik
...
pxi이 만든 Cloudflare 리소스는 자동으로 안 지웁니다.

```bash
# DNS 레코드 일람
pxi run cloudflare dns-list --domain example.com

# 개별 삭제
pxi run cloudflare dns-delete --domain example.com --name myapp --type A
...
`pxi run connect set/encrypt`로 만든 `.env` + `.env.vault` + `.env.keys`가 프로젝트 디렉토리에 남아 있습니다.

```bash
# 보통 위치
ls -la /etc/pxi/.env*       # 시스템
ls -la ~/.config/pxi/.env*  # 사용자
...
`pxi uninstall --purge`는 표준 위치만 지웁니다. 다른 곳의 .env.vault는 안 건드림.

---

## 8. systemd timers/services

`pxi install ai`, `pxi run ai openclaw-setup` 등이 systemd 유닛을 등록할 수 있습니다.

```bash
# pxi이 등록한 유닛 목록
sudo systemctl list-unit-files | grep -E "pxi|cluster-files|adversarial"
...
`pxi install bootstrap`이 시스템 패키지/바이너리를 설치했습니다. 다른 용도로도 쓸 수 있어 자동 제거 안 합니다.

**먼저 manifest로 pxi이 *설치할 수 있는* 항목 카탈로그를 확인:**

```bash
pxi run bootstrap manifest             # 사람용 표
pxi run bootstrap manifest --json      # 자동화/스크립트용
pxi run bootstrap manifest --only gh   # 특정 도구만
```

⚠️ **manifest는 정적 카탈로그입니다** — pxi install이 idempotent라서, 이미 호스트에 있던 도구는 pxi이 깐 것이 아닐 수 있습니다. `detected_on_host=true` 표시는 단순히 "PATH에 명령이 있다"는 뜻일 뿐, pxi이 출처라는 보장은 아닙니다.

각 항목은 **pxi install이 깔 수 있는 후보** + **pxi remove가 실제로 하는 동작**입니다.
...
[gh] (detected=true)
  ...
  부가 파일:            /etc/apt/sources.list.d/github-cli.list,
                        ...
                        ~/.config/gh/
  pxi remove --only gh 이 실제로 하는 것:
    sudo apt-get remove -y gh
    sudo rm -f /etc/apt/sources.list.d/github-cli.list \
               /usr/share/keyrings/githubcli-archive-keyring.gpg
  사용자 수동 후속 정리 (pxi이 자동 안 함):
    rm -rf ~/.config/gh    # 인증 토큰 포함 — 사용자 결정
```

manifest의 **`pxi_remove_does`** 절차는 `pxi run bootstrap remove --only X`가 실제 수행하는 동작과 정확히 일치하며, **`manual_followup`** 은 pxi이 자동으로 안 지우는 잔여물입니다. 가능하면 직접 명령 대신 `pxi run bootstrap remove --only <tool>`을 사용하세요. JSON 사용 시 `schema_version`으로 분기 (현재 = 2).

추가 주의:
- **apt**: pxi의 `remove --only apt`는 `build-essential`만 제거합니다 (curl/git/jq 등 시스템 핵심은 보존). manifest의 `static_install_packages`를 그대로 `apt remove --purge`하지 마세요.
- **dotenvx**: pxi이 nodejs/npm까지 깔았을 수 있지만 다른 도구도 쓰는 의존이라 자동 제거 안 합니다.
...
```bash
# 바이너리 흔적
which pxi pxi-lxc pxi-ai 2>&1 | grep -v "no.*in"

# config/state 흔적
ls -la ~/.config/pxi /etc/pxi /var/lib/pxi 2>&1 | grep -v "No such"

# systemd 흔적
sudo systemctl list-unit-files 2>/dev/null | grep pxi

# fstab/cifs/postfix 흔적
sudo grep pxi /etc/fstab
...
`--purge` 했어도 복구 가능:
- LXC/VM은 vzdump 백업으로 복원 (`pxi run backup restore` 또는 `pct restore`)
- recovery snapshot으로 LXC config만 복원: `pxi run recovery list/restore`
